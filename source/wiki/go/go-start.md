...
三段式中的每个部分都是可省略的，通过这点来实现 `while` 循环和死循环
...
> go 支持 `break` 和 `continue` 关键字用于控制循环，支持标签和 `goto` 关键字实现跳转，但通常不会使用标签和 `goto`，这会导致复杂的代码逻辑
...
引用类型用于复杂数据，使用存储在栈上的指针指向存储在堆中的数据，引用类型赋值时拷贝引用，常见的引用类型有切片、map、chan

> 以上模式与 java 基本相同

go 支持指针类型，指针属于值类型，可以获取一个变量的指针（内存地址），可以通过指针修改值，但不能进行指针运算
